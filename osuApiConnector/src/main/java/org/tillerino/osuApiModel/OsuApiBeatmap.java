...
public class OsuApiBeatmap {
    ...
    public static double calcBpm(double bpm, @BitwiseMods long mods) {
        if (DoubleTime.is(mods)) bpm *= 1.5;
        if (HalfTime.is(mods)) bpm *= 0.75;
        return bpm;
    }

    public static int calcTotalLength(int totalLength, @BitwiseMods long mods) {
        if (DoubleTime.is(mods)) totalLength = (int) (totalLength * 2D / 3);
        if (HalfTime.is(mods)) totalLength = (int) (totalLength * 4D / 3);
        return totalLength;
    }
